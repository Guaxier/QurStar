...
CREATE TABLE ba_Role_Permissions(
    ...
    PermissionID INT NOT NULL,
    PRIMARY KEY(RoleID, PermissionID),
    FOREIGN KEY(RoleID) REFERENCES ba_Roles(RoleID) ON DELETE CASCADE,
    FOREIGN KEY(PermissionID) REFERENCES ba_Permissions(PermissionID) ON DELETE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '角色与权限关联表'; 
...
-- 外键约束单独添加，确保表创建成功后再添加约束
ALTER TABLE
    `ba_users` ADD CONSTRAINT `fk_users_roles` FOREIGN KEY(`user_role`) REFERENCES `ba_Roles`(`RoleID`);
    -- 为user_role创建索引以优化查询
CREATE INDEX `idx_user_role` ON
    `ba_users`(`user_role`);

-- 登录表
CREATE TABLE ba_UserLoginRecords(
    -- 主键，自增
    LoginRecordID INT AUTO_INCREMENT PRIMARY KEY COMMENT '登录记录ID',
    -- 用户关联，与user表中的user_id关联
    user_id INT NOT NULL COMMENT '关联用户ID',
    FOREIGN KEY(user_id) REFERENCES ba_users(user_id) ON DELETE CASCADE,
    -- 记录用户登录方式
    Login_Method VARCHAR(255) COMMENT '登录方式',
    -- 登录设备类型和操作系统信息
    Device VARCHAR(255) COMMENT '登录设备',
...
-- 曾用邮箱表
CREATE TABLE ba_UserEmailHistory(
    -- 主键，自增
    UserEmailHistoryID INT AUTO_INCREMENT PRIMARY KEY COMMENT 'ID',
    -- 用户关联，与user表中的user_id关联，级联删除
    user_id INT NOT NULL COMMENT '关联用户ID',
...
-- 曾用户名表
CREATE TABLE ba_UsernameHistory(
    -- 主键，自增
    UsernameHistoryID INT AUTO_INCREMENT PRIMARY KEY COMMENT 'ID',
    -- 用户关联，与user表中的user_id关联，级联删除
    user_id INT NOT NULL COMMENT '关联用户ID',
...
-- 曾用手机号表
CREATE TABLE ba_UserPhoneHistory(
    -- 主键，自增
    UserPhoneHistoryID INT AUTO_INCREMENT PRIMARY KEY COMMENT 'ID',
    -- 用户关联，与user表中的user_id关联，级联删除
    user_id INT NOT NULL COMMENT '关联用户ID',
...
-- 曾用密码表
CREATE TABLE ba_PasswordHistory(
    -- 主键，自增
    PasswordHistoryID INT AUTO_INCREMENT PRIMARY KEY COMMENT 'ID',
    -- 用户关联，与user表中的user_id关联
    user_id INT NOT NULL COMMENT '关联用户ID',
